...
def scrape_article(url: str) -> str:
    ...
    logger.debug(f"Extracted text length from {url}: {len(cleaned_text)} chars")

    snippet = cleaned_text[:500] + ("..." if len(cleaned_text) > 500 else "")
    logger.debug(f"Extracted text snippet from {url}:\n{snippet}")
